...
public class OngoingFragment extends Fragment {
    ...
    List<SuccessModelVendor> overViewModules;
                    ...
                    if (response.body().getSuccess().getData().size() != 0) {
                        ...
                        for(int i=0;i<response.body().getSuccess().getData().size();i++) {
                            if (response.body().getSuccess().getData().get(i).getStatus().equals("inprocess") ||
                                ...
                                if (response.body().getSuccess().getData().get(i).getStatus().equals("paymentDone")) {
                                    sampleData.setTransactionModel(response.body().getSuccess().getData().get(i).getTransactionModel());
                                }
